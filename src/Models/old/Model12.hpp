...
public:
	...
	Model12(const bool allSeasons) {
		this->teamMap = TEAMS_2007_2012;
	}
	Model12(const DoubleVector& logR, const double h, const double a, const DoubleVector& c,
			const DoubleVector& d, const DoubleVector& U, const double A, const double B, const double C,
...
inline Model12::Model12(const DoubleVector& logR, const double h, const double a, const DoubleVector& c,
...
inline DoublePair Model12::rate(const double t, const int homeScore, const int awayScore,
	...
	double betaHome = importanceHomeAway.first > 0.0 ? logistic(A + B * homeMatchesPlayed) : 0.0;
	double betaAway = importanceHomeAway.second > 0.0 ? logistic(A + B * awayMatchesPlayed) : 0.0;
...
inline DoublePair Model12::rateIntegral(const double start, const double end, const int homeScore,
		const int awayScore, const Team& homeTeam, const Team& awayTeam, const League& league) const {
...
inline void Model12::calculateAlphaLeague(DoublePair& alphaHomeAway, DoublePair& importanceHomeAway,
...
inline void Model12::setParameters(const DoubleVector& theta) {
...
inline double wrapperModel12(const DoubleVector& theta, DoubleVector& grad, void* modelDataAndSeason) {
	...
	ModelDataAndSeason<Model12>* m = static_cast<ModelDataAndSeason<Model12>* >(modelDataAndSeason);

	Model12 model = m->model;
...
#ifdef HAVE_NLOPT
inline void Model12::findMLE(DoubleVector& theta, const Data& data, const Season& season) {

	double f;
	nlopt::opt opt(nlopt::LN_SBPLX, 39);

	ModelDataAndSeason<Model12> modelDataAndSeason;
	modelDataAndSeason.model = *this;
	modelDataAndSeason.data = data;
	modelDataAndSeason.season = season;

	opt.set_max_objective(wrapperModel12, &modelDataAndSeason);
...
inline void Model12::setParametersAllSeasons(const DoubleVector& theta, const bool withKnots) {
...
inline double wrapperModel12AllSeasons(const DoubleVector& theta, DoubleVector& grad, void* modelAndAllData) {
	...
	ModelAndAllData<Model12>* m = static_cast<ModelAndAllData<Model12>* >(modelAndAllData);
...
#ifdef HAVE_NLOPT
inline void Model12::findMLE(DoubleVector& theta, const vector<Data>& allData) {

	double f;
	nlopt::opt opt(nlopt::LN_SBPLX, 48);

	ModelAndAllData<Model12> modelAndAllData;
	modelAndAllData.model = *this;
	modelAndAllData.allData = allData;

	opt.set_max_objective(wrapperModel12, &modelAndAllData);
	opt.set_ftol_rel(1e-12);
	opt.set_maxeval(100000);
