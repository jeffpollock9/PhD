...
public:
	...
	void calculateAlphaLeague4(DoublePair& alphaHomeAway, DoublePair& importanceHomeAway, const int homeScore,
...
private:
	...
	DoubleVector startingValues = { 0.293642, -0.0357756, 0.347895, 0.4171, 0.499926, 0.711756, 0.556388,
			...
			0.35111, 0.373068, 0.298448, 0.213093, -5.10682, 0.11722, 11.0, 9.0, 8.0, 7.0, 7.5, 6.0, 1.0 };
};

inline void Model4::calculateAlphaLeague4(DoublePair& alphaHomeAway, DoublePair& importanceHomeAway,
	...
	for (const int p : Phome) {
		double homeDiff = utilityModel.at(p) - currentHomeUtility;
		homeAlphaSum += homeDiff;
		homeImpSum += abs(homeDiff);
	}

	for (const int p : Paway) {
		double awayDiff = utilityModel.at(p) - currentAwayUtility;
...
inline DoublePair Model4::rate(const double t, const int homeScore, const int awayScore, const Team& homeTeam,
		const Team& awayTeam, const League& league, const bool isLog) const {
	...
	calculateAlphaLeague4(alphaLeagueHomeAway, importanceHomeAway, homeScore, awayScore, homeTeam, awayTeam,
			league);

	int homeMatchesPlayed = league.matchesPlayedForTeam(homeTeam);
	int awayMatchesPlayed = league.matchesPlayedForTeam(awayTeam);

	double betaHome = logistic(A + B * homeMatchesPlayed * importanceHomeAway.first);
	double betaAway = logistic(A + B * awayMatchesPlayed * importanceHomeAway.second);
...
inline DoublePair Model4::rateIntegral(const double start, const double end, const int homeScore,
		const int awayScore, const Team& homeTeam, const Team& awayTeam, const League& league) const {
...
inline void Model4::setParametersAllSeasons(const DoubleVector& theta) {
...
inline double wrapperModel4(const DoubleVector& theta, DoubleVector& grad, void* modelAndAllData) {
	...
	ModelAndAllData<Model4>* m = static_cast<ModelAndAllData<Model4>*>(modelAndAllData);

	Model4 model = m->model;
...
inline void Model4::findMLE(DoubleVector& theta, const vector<Data>& allData) {

	double f;
	nlopt::opt opt(nlopt::LN_SBPLX, nParameters);

	ModelAndAllData<Model4> modelAndAllData;
	modelAndAllData.model = *this;
	modelAndAllData.allData = allData;

	opt.set_max_objective(wrapperModel4, &modelAndAllData);
